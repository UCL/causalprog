...
def test_maximise():
    min_solution = augmented_lagrangian(f, {"x1": 0.0, "x2": 0.0}, bounds)
    max_solution = augmented_lagrangian(
        lambda x: 5.0 - f(x),
        {"x1": 0.0, "x2": 0.0},
        bounds,
        max_or_min="max",
...
def test_bounds(bounds_function, expected_minimum, expected_maximum):
    min_solution = augmented_lagrangian(f, {"x1": 0.0, "x2": 0.0}, bounds_function)
