...
and used, and is aimed at users of the library. Documentation for library
developers can be found in [the documentation for
developers](../developers/graph.md).
...
automatically added to the graph when the node is added. For example, in the
following snippet a data node representing a scalar and a continuous random
variable node are added to the graph.
...
Distrubution nodes represent the values of random variables (RVs) that can
...
`DistributionNode`s. This method formed part of an earlier experimental
version of the library and is not used in the current demonstration
applications.

## Graph algorithms

The causalprog library includes a number of algorithms that can be applied
to graphs that it has created.
...
node the do is applied to, and the value imposed on that node. It may also
...
These algorithms formed part of an earlier experimental version of the
library and are not used in the current demonstration applications.
